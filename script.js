...
const enemyCars = []; // modelos prontos em cache
// Pré-carregar modelo de carro inimigo
enemyLoader.load("models/carro_inimigo.glb", (gltf) => {
  enemyCars.push(gltf.scene);
});
...
const backgroundTexture = textureLoader.load("textures/ceu.jpeg");
...
function criarPredio(x, z) {
  const texturaPredio = textureLoader.load("textures/texturaPredio.jpg");
...
loader.load("models/carro.glb", (gltf) => {
  player = gltf.scene;
...
function animate() {
  ...
  for (let i = obstacles.length - 1; i >= 0; i--) {
    ...
    // Mover na direção do jogador (eixo Z)
    ...
    // Remove obstáculos que passaram do jogador
  ...
  if (obstacleSpawnCounter >= obstacleSpawnInterval && enemyCars.length > 0) {
    obstacleSpawnCounter = 0;

    const numObstacles = Math.floor(Math.random() * 2) + 2;

    for (let i = 0; i < numObstacles; i++) {
      const clone = enemyCars[0].clone(true); // clona o modelo carregado
      clone.scale.set(1.2, 1.2, 1.2);
      clone.rotation.y = Math.PI / 2;

      const posX = (Math.random() - 0.5) * 24;
      const posZ = player.position.z - 100 - Math.random() * 30;

      // Aqui está a correção:
      clone.position.set(posX, player.position.y + 0.641, posZ);
      ...
      clone.userData.speed = gameSpeed * 0.6 + 0.1;

      // Ativa sombras nos meshes filhos
